...
# output file
output_file = "../data/02_replication/timeTotal/totalTimeMetrics.csv"
...
for i in range(len(dataframes)):
    ...
    ax.bar(get_pretty_name(columnNames[i]), duration.total_seconds() / 60, label=get_pretty_name(columnNames[i]))
ax.set_ylabel('Time (minutes)')
ax.set_ylim(bottom=0)  # Limit y-axis to minimum value of 0
ax.set_title('Duration for 5 million requests')
ax.grid(axis='y')
plt.savefig(os.path.join(figures_directory, f'duration_5mio.png'))
plt.show()
