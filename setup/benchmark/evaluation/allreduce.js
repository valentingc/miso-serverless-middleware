...
(function (BenchmarkType) {
    BenchmarkType["miso"] = "miso";
    BenchmarkType["redis"] = "redis";
    BenchmarkType["minio"] = "minio";
})(BenchmarkType || (BenchmarkType = {}));
const TYPE = BenchmarkType[args[0]] ?? BenchmarkType.miso;
...
const RESULT_FILE_PATH = '/data/json/benchmark_results_' +
    TYPE +
    '_' +
    ITERATIONS +
    'ITER_' +
...
async function runSingleTask(chunk, timeStart, idx) {
    try {
        const response = await axios.post('http://gateway.openfaas.svc.cluster.local:8080/function/allreduce-' +
            TYPE, {
...
async function init(timeStart) {
    switch (TYPE) {
        case BenchmarkType.minio:
            console.log('Performing init-minio');
            const { data } = await axios.post('http://gateway.openfaas.svc.cluster.local:8080/function/allreduce-' +
                TYPE, {
                operation: 'init-' + TYPE,
                crdtName: 'pncounter-' + String(timeStart),
                minio: {
...
async function performTest(n, timeStart) {
    await init(timeStart);
    const timeStartAfterInit = process.hrtime.bigint();
    console.log('Init took ' + Number(timeStartAfterInit - timeStart) / 1e6 + 'ms');
    const lowEnd = 0;
    ...
    const timeExecution = results.reduce((prev, current) => prev.add(new Decimal(current.times.durationExecution)), new Decimal(0));
    return {
        isCorrect: isCorrect,
...
async function cleanup(timeStart) {
    if (TYPE === 'minio') {
        const { data } = await axios.post('http://gateway.openfaas.svc.cluster.local:8080/function/allreduce-' +
            TYPE, {
