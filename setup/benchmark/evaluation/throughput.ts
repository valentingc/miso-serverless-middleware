...
const setEnvVariablesForNode = (node: string) => {
...
const statefulObjects: StatefulObjectProxy[] = [];
const nodeNameIpMap = new Map<string, LookupAddress>();
const getIpAddress = async (nodeName: string) => {
  try {
...
const register = async () => {
  await Promise.all(
    nodeNames.map(async (node, index) => {
...
const getPayload = (type: string, podName: string) => {
...
getAllIpAddreses().then(() => {
  register().then(() => {
    ...
    if (CONF_TEST_TYPE === 'distributed') {
      ...
      ghzTestProcess.on('exit', function (code) {
        console.log(
          CONF_IP_AND_PORT + ' - child process exited with code ' + code,
        );
      });
    } else {
...
const runXTestsInParallel = async (x: number) => {
