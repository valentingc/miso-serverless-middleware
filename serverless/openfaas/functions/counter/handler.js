...
async function getPNCounterResponseEvaluation(body, statefulObject, operation) {
    ...
    let finalResult = { acknowledged: true };
...
async function getGCounterResponse(body, statefulObject) {
    const counter = statefulObject.getGCounter('gcounter1');
    try {
        const now = Date.now();
        let result = await counter.add(body.value);
        console.log(`GCounter Add Request finished... ${Date.now() - now}ms, Response: ${JSON.stringify(result)}`);
        result = await counter.getValue();
        console.log(`GCounter GetValue finished... ${Date.now() - now}ms, Response: ${JSON.stringify(result)}`);
        return result;
    }
    catch (error) {
        console.error(error);
...
async function getPNCounterResponse(body, statefulObject) {
    const counter = statefulObject.getPNCounter('pnCounter1');
    try {
        const now = Date.now();
        let result = await counter.add(body.value + 1);
        console.log(`PNCounter Add Request finished... ${Date.now() - now}ms, Response: ${JSON.stringify(result)}`);
        result = await counter.getValue();
        console.log(`PNCounter GetValue finished... ${Date.now() - now}ms, Response: ${JSON.stringify(result)}`);
        result = await counter.subtract(body.value);
        console.log(`PNCounter Subtract finished... ${Date.now() - now}ms, Response: ${JSON.stringify(result)}`);
        return result;
    }
    catch (error) {
        console.error(error);
