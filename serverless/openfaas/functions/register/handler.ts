...
function generateStringOfSize(size: string) {
...
async function getMVRegisterResponseConcat(
  body: any,
  statefulObject: StatefulObjectProxy,
) {
  const register = statefulObject.getMVRegister<string>(
    'MVRegister' + body.size + Date.now() + '',
  );
  body.append = body.append + '';

  const originalRegisterValue = await getRegister(
    statefulObject,
    body.size,
  ).getValue();
...
const getRegister = (statefulObject: StatefulObjectProxy, size: string) => {
  return statefulObject.getMVRegister<string>('MVRegister' + size);
};

async function getInitResponse(body: any, statefulObject: StatefulObjectProxy) {
...
async function getMVRegisterResponse(
  body: any,
  statefulObject: StatefulObjectProxy,
) {
  ...
  try {
    console.log('NUMBER');
    let now = Date.now();
    const mvregisterNumber =
      statefulObject.getMVRegister<number>('MVRegisterNumber');
    let result: number[] | string[] | object[] = await mvregisterNumber.assign(
      1,
    );
    console.log(
      `MVRegister assign Request finished... ${
        Date.now() - now
    ...
    console.log(
      `MVRegister GetValue finished... ${
        Date.now() - now
      }ms, Response: ${JSON.stringify(resultNumber)}`,
    );

    const mvregisterString =
      statefulObject.getMVRegister<string>('MVRegisterString');
    console.log('STRING');
    now = Date.now();
    ...
    const mvregisterObject =
      statefulObject.getMVRegister<object>('MVRegisterObject');
    console.log('OBJECT');
    now = Date.now();
    ...
    console.log(
      `MVRegister GetValue finished... ${
        Date.now() - now
      }ms, Response: ${JSON.stringify(resultObject)}`,
    );
